  ...
  return (
    <div className="App">
      ...
      <div className='footercontainer'>
        <div>
          <a target={'_blank'} href={'https://github.com/panley01/site'} style={{ color: 'white' }}><h1 style={{ color: 'white' }}>{<FaGithub />}{((process.env.PUBLIC_URL.includes('levels.pnly.io')) ? '  This website is a parody!' : '  This website is OSS!')}</h1></a>
          <a target={'_blank'} href={'https://pages.cloudflare.com/'} style={{ color: 'white' }}><h2 style={{ color: 'white' }}>{<FaCloudflare />}{((process.env.PUBLIC_URL.includes('levels.pnly.io')) ? '  Hi Twitter.' : '  And hosted using Cloudflare Pages')}</h2></a>
        </div>
      </div>
    </div>
  );
